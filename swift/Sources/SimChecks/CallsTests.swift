...
///
/// Measured across the first three of the eleven-match pool: 1, 2 and 1 calls
/// (mean 1.3), and 2.8 over the whole pool — roughly two receiving fouls and
/// the odd pick or strip per match. Travels remain at zero, which is the pivot
/// work's number and is left alone here.
///
/// The first-three mean has read under 5, then 6.0, now 1.3 across the
/// throw-solver and column changes of issues #65 and #64, while the detectors
/// stood untouched (the contact geometry and contest judgement are still
/// bit-exact against the reference in `RulesTests`) — the three-seed mean is
/// a three-sample tail, up and down, and the property lives on the
/// eleven-pool mean, which has read 3.4, 4.6 and now 2.8, always under the
/// same bar of 5. The systematic failure this ceiling exists for ran 8.3 a
/// match on every seed.
///
