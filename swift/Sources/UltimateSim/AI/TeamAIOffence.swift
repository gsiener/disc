...
extension TeamAI {
    ...
    func cutterIntent(_ p: AIPlayer, _ world: AIWorld, _ dt: Double) -> PlayerIntent {
        ...
        case .clear:
            ...
            // reset is for — `stackAxisX` is mid-field until that would stand in its lane, then break-side of the disc and the clear
            // steers at `p.pos.z + dir * 1.2`, so a reset finishing a dump on the open
            // side was sent across the field and FORWARD, past the disc, on every dead
            ...
            if p.role == .handler || !Playbook.hasColumn(formation) {
                ...
                // `stackAxisX` is mid-field until that would stand in its lane, then break-side of the disc for every set that is not a
                // side stack — so steering everybody at it collapsed a 22 m-wide endzone
                // set into a knot off the disc on every dead cut, in the one situation the
