...
public struct Playbook: Sendable {
    ...
    public func formationStations(
        ...
        //
        // PIN_MARGIN IS A STAND-OFF FROM WHATEVER THE BACKFIELD IS PINNED AGAINST, and
        // until issue #29 only one of the two things it can be pinned against was written
        // down.
        //
        // *It is the BACKFIELD's floor, so it is applied by role.* Cutter stations are
        ...
        // is on `dump` and `swing` only, never on a downfield cut.
        //
        // *And the floor is a stand-off from the DISC too, once the disc is the thing the
        // backfield is pinned against.* On the line the two readings are the same number:
        // the disc is at -32, the floor is at -30, and the handlers stand two metres in
        // front of the thrower. Thirteen metres deeper into the endzone the absolute
        // floor stops describing that at all — it stands the backfield fifteen metres in
        // front of the thrower, which is not a backfield, it is a second cutter row. And
        // that is literally what it collided with: the ho cutter row is `a.z + dir * 15`,
        // so with the disc at z = -45 both rows are at -30 and a handler and a cutter
        // come back as ONE POINT, exactly, at x = -5.5.
        //
        // So the backfield stands `pinMargin` in front of its own goal line or
        // `pinMargin` in front of the disc, whichever is deeper. The second clause is
        // inert whenever the disc is in front of its own goal line — `a.z + pinMargin >=
        // floorZ` there by construction — so nothing on the playing field proper moves,
        // and `PIN_MARGIN` keeps the meaning `buildCut` already relies on.
        //
        ...
        let backZ =
            dir > 0
            ? Swift.min(floorZ, a.z + pinMargin)
            : Swift.max(floorZ, a.z - pinMargin)

        func push(_ x: Double, _ z: Double, _ role: StationRole, _ depth: Int) {
            let zz =
                role != .handler
                ? z
                : (dir > 0 ? Swift.max(z, backZ) : Swift.min(z, backZ))
