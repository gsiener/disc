...
export function formationStations(
   ...
   *
   * THE FLOOR IS THE BACKFIELD'S, AND IT RUNS OUT OF ROOM BEFORE THE PITCH
   * DOES. Two separate things, both wrong until issue #29, and the sentence
   * above is the contract they are both meant to serve.
   *
   * *It is the BACKFIELD's, so it is applied by role.* Cutter stations are
   * `a.z + dir * ...`, so they are always downfield OF THE DISC — which keeps
   * them clear of a floor fixed at the goal line for exactly as long as the
   * disc is in front of it, and no longer. Behind the line the disc is itself
   * below the floor, so the floor stops being a floor and becomes a magnet,
   * dragging the front of the stack forward until two stations arrive as the
   * same point. Measured off this builder, disc on the centre line, dir +1: the
   * side stack's front two cutters coincide from z = -43.2 and the vertical
   * stack's from z = -45.2 (minis: -16.875 and -17.65625). `buildCut` draws
   * this line by role already — its floor is on `dump` and `swing` only, never
   * on a downfield cut. `PIN_MARGIN` is untouched by this and keeps its one
   * meaning: how far in front of its own goal line the BACKFIELD sets up.
   *
   * *#29 also shipped a "mirror": a floored handler stood in front of the disc
   * (`2 * a.z - z`) rather than at the line, to keep it a real distance from
   * the thrower instead of parked on the mark. That part reads as it should —
   * `back = min(floorZ, mirrored)` for dir > 0, so the smaller (deeper) of the
   * two wins. But a handler's offset off the disc tops out around 6.5 m, so
   * whenever the mirror actually clears the floor (`a.z + off >= floorZ`),
   * `mirrored >= floorZ` and the min picks `floorZ` anyway — the mirror was
   * live only in the one regime it should not have been: a disc more than
   * 6.5 m behind the floor (a real pull can land inside 6 m of the end line),
   * where the mirror falls SHORT of the floor and the min then picks
   * `mirrored`, a station behind the floor `PIN_MARGIN` exists to rule out.
   * `formationStations` never actually mirrored the case it was written for;
   * it only ever un-floored the case it was written to still floor.
   *
   * Issue #35: measured on the `SEED=20260729` sevens match `tools/test-
   * game.ts` plays, turnovers per point went from 0.88 (floor by role alone,
   * no mirror) to 3.17, and seed 33333's pooled completion rate moved from
   * 67% to 58% — both outside their bands, and both because the receiving
   * team's backfield was standing in a genuinely bad spot after a deep pull,
   * not because of an unlucky roll. Reference first, per ADR-0001: dropping
   * the mirror back to a plain per-role floor is what `tools/test-game.ts`
   * and `tools/test-ai.ts` measure as correct — every check both suites had
   * passing before #29's second commit passes again, including the ones the
   * mirror was quietly failing without a band to catch it: `test-ai.ts`'s
   * `no out-of-bounds across seeds` (26 player-ticks with the mirror live,
   * 0 without — a defender chasing a mirrored station off the field) and
   * `a reset handler is stationed behind the disc` (a mirrored handler
   * stands in FRONT of the disc by definition, which is not behind it by any
   * distance; dropping the mirror is what let this recover from a station
   * that was never legitimately live in the case that mattered anyway).
   *
   * What the mirror actually bought — avoiding the ho set's handler landing
   * exactly on its cutter row at disc (-5.5, -45) sevens — was a side effect
   * of the SAME bug: the un-floored `mirrored` value happens not to equal
   * `a.z + dir * 15` there, but it bought that by standing the handler
   * anywhere up to 6.5 m into the offence's own endzone. Dropping the mirror
   * brings the near-miss back, not the exact collision it started as:
   * `PlaybookTests.minisShape`'s pairwise-distinct sweep measures it, with
   * this floor, at 0.1300 m at sevens and 0.0528 m at minis — both real, both
   * a body short of the 0.75 m the sweep otherwise wants, and both confined
   * to the one case with no shared row to slide: the ho handler clamps to an
   * absolute `floorZ` while its cutter row stays disc-relative (`a.z + dir *
   * 15`), so the two cross once, at a specific disc position, with no
   * `rowShift`-shaped fix available short of resurrecting the mirror (and
   * its 3.17 turnovers a point) or nudging the cutter row too — untried, and
   * out of scope for #35. See that test for the accepted, documented bound.
   *
   * The alternative — capping the floor at the disc plus `PIN_MARGIN` — was
   * already measured and rejected by #29: two metres in front of a pinned
   * thrower is where the MARK stands, and `tools/test-ai.ts` went from
   * `marker respects 1 m disc space  closest approach 1.75m` to `0.70m`.
   *
   ...
   * whose domain runs to the offence's own END line rather than its goal line.
   */
  const floorZ = -dir * (FIELD.goalLine - PIN_MARGIN);
  const push = (x: number, z: number, role: 'handler' | 'cutter', depth: number): void => {
    const zz = role === 'handler' ? (dir > 0 ? Math.max(z, floorZ) : Math.min(z, floorZ)) : z;
