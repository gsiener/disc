...
export class AudioSystem implements System {
  ...
  start(): boolean {
    ...
    try {
      ...
      if (typeof phase === 'string') { this.crowd.setPhase(phase); this.lastPhase = phase; }
  ...
  private wireEvents(ctx: Ctx): void {
    ...
    this.on(ctx, 'disc:released', (p) => {
      ...
      this.flight?.release(x, y, z, clamp(speed / 26, 0, 1));
      // A huck is an event in itself — the stands react to the shape of the
      // throw before they know whether it lands.
      if (speed > 19) this.crowd?.react('huck', clamp((speed - 19) / 10, 0.3, 1.2));
    });
    ...
    this.on(ctx, 'disc:caught', (p) => {
      ...
      if (out === 'goal') return;                       // 'score' handles the roar
      if (out === 'interception') this.crowd?.react('interception');
      else if (out === 'pull') this.crowd?.react('catch', 0.6);
      else this.crowd?.react(airborne ? 'bigCatch' : 'catch');
    });
    ...
    this.on(ctx, 'turnover', (p) => {
      ...
      else if (r === 'drop') this.crowd?.react('drop');
      else this.crowd?.react('turnover');
    });

    this.on(ctx, 'stall:tick', (p) => {
      const c = num(p?.count, 0);
      const max = Math.max(1, num(p?.max, 10));
      // The stands only join a stall count near the end of it.
      if (c >= max - 3) this.crowd?.react('stall', (c - (max - 4)) / 4);
    });

    this.on(ctx, 'state:changed', (p) => {
      const to = String(p?.to ?? '');
      if (to) { this.crowd?.setPhase(to); this.lastPhase = to; }
    });
    ...
    this.on(ctx, 'player:land', (p) => {
      ...
      if (layout) this.crowd?.react('layout', 0.8);
    });
    ...
    try {
      ...
      if (this.flight && st) {
        ...
        this.flight.update(d, {
          x: num(p?.x, 0), y: num(p?.y, 1), z: num(p?.z, 0),
          airspeed: num(st.airspeed, 0),
          spin: num(st.spin, 0),
          inFlight: mode === 'flight' && !st.atRest,
        });
