...
| major | `20260811-a-named-constant-is-invisible` | Adding `export` to a reference constant deletes it from the divergence registry, and the failure names the wrong cause |
| major | `20260811-the-aim-height-was-repaired` | A callee that silently repairs its caller''s bad input makes the input unobservable: 1699 of 1699 throws asked for an unreachable aim plane with every fixture green |
| major | `20260811-matchdiff-golden-is-stale` | matchdiff.json is stale on main, so any agent who regenerates it inherits somebody else''s behaviour change and a red suite |
